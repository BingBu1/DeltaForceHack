...
namespace Dumper
{
	...
	bool InitSdk(const std::wstring& moduleName, uintptr_t gObjectsOffset, uintptr_t gNamesOffset, uintptr_t gWorldOffset)
	{
		...
		UObject::GObjects = reinterpret_cast<Dumper::TUObjectArray*>(mBaseAddress + gObjectsOffset);
		FName::GNames = reinterpret_cast<Dumper::FNamePool*>(mBaseAddress + gNamesOffset);
		UWorld::GWorld = reinterpret_cast<Dumper::UWorld**>(mBaseAddress + gWorldOffset);
	...
	bool InitSdk()
	{
		return InitSdk(L"DeltaForceClient-Win64-Shipping.exe", GOBJECT_OFFSET, GNAME_OFFSET, GWORLD_OFFSET);
	}
	...
	{
		int block = key >> 18;
		uint32_t offset = key & 0x0003FFFF;
